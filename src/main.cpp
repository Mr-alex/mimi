...
uint256 hashGenesisBlock("0xbecdc23ec5ce5000ebb72199465c78b5198a4a29af3447502eb0613fef22a908");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // mimicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "mimicoin Signed Message:\n";
...
{
    // mimicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 60; // mimicoin: 10 minutes
static const int64 nTargetSpacing = 120; // mimicoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // mimicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // mimicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbecdc23ec5ce5000ebb72199465c78b5198a4a29af3447502eb0613fef22a908");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        const char* pszTimestamp = "mimicoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 88 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b2cb32c74b2e6df597590c58d842e1065953d3b352001d681d0fb82b5fdc60f166aa1e5544b08eccf35b2b7667326a1ac28c78aa02a7d8e20b49e07cda3f0096") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0x18630b6c82bf4680bb7bbc686ffd4cc14d47fff0c20a916517cb109be5df7235"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // mimicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("mimicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("mimicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static mimicoinMiner(CWallet *pwallet)
{
    printf("mimicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mimicoin-miner");
    ...
    try { loop {
        ...
        printf("Running mimicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("mimicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&mimicoinMiner, pwallet));
}
